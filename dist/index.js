...
var hotelBooking = mysqlTable("hotelBooking", {
  id: int("id").autoincrement().primaryKey(),
...
async function criarLinkInfinitePay() {
...
var appRouter = router({
  ...
  hotelBooking: router({
    ...
    update: publicProcedure.input(z2.object({
      ...
      checkInDate: z2.string().optional(),
      checkOutDate: z2.string().optional(),
      hospedageValue: z2.number().optional(),
      paymentLink30: z2.string().optional(),
      clientEmail: z2.string().optional(),
      clientPhone: z2.string().optional(),
      guestCount: z2.number().optional(),
      detail1: z2.string().optional(),
      detail2: z2.string().optional(),
      detail3: z2.string().optional(),
      roomType: z2.string().optional(),
      breakfastIncluded: z2.boolean().optional(),
      freeCancellationDate: z2.string().optional(),
      mainGuestName: z2.string().optional(),
      photos: z2.string().optional()
    })).mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const existing = await db.select().from(hotelBooking).limit(1);
      if (existing.length > 0) {
        await db.update(hotelBooking).set({
          ...input,
          breakfastIncluded: input.breakfastIncluded ? 1 : 0
        }).where(eq2(hotelBooking.id, existing[0].id));
